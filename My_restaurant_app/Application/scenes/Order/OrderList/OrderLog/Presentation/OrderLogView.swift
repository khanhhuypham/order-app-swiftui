...
struct OrderLogView: View {
    ...
    init(orderId: Int) {
        ...
        let repository = OrderLogRepositoryImpl()
        let useCase = GetOrderLogUseCase(repository: repository)
        _viewModel = StateObject(wrappedValue: OrderLogViewModel(getOrderLogUseCase: useCase))
    }
